...
namespace Christina.UI
{
    public class ToggleSwitch : MonoBehaviour, IPointerClickHandler
    {
        ...
        {
            ApplyState(value, animate: false, notify: false);
        }

        public void SetIsOn(bool value)
        {
            ApplyState(value, animate: true, notify: true);
        }
        
        
        private void SetStateAndStartAnimation(bool state)
        {
            ApplyState(state, animate: true, notify: true);
        }

        private void ApplyState(bool state, bool animate, bool notify)
        {
            _previousValue = CurrentValue;
            CurrentValue = state;

            var valueChanged = _previousValue != CurrentValue;
            if (valueChanged)
            {
                if (CurrentValue)
                    onToggleOn?.Invoke();
